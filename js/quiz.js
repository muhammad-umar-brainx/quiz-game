...
let result = $("#result");
let totalQuestionsToShow = 4;
let optionPerEachQuestion = 3;
let randomQuestions = [];
...
function insertQuestion(questionIndex, index, arr) {
  ...
  optionsOfEachQuestion = generateArrayOfRandom(97, 101, optionPerEachQuestion-1, optionsOfEachQuestion);
...
function showResult() {
  ...
  $('input:radio:checked').each(function () {
    if (this.value == myQuestions[this.name].correctAnswer) {
      $(`h1#${this.name}`).removeClass("text-danger");
  ...
  if (correct >= minQuestionToBeCorrected) {
    Swal.fire({title: "Good job", text: 'Correct Answers are ' + correct, type:
        "success"}).then(function(){
        location.reload();
      }
    );
  } else {
    Swal.fire({title: "Oops", text: 'Correct Answers are ' + correct, type:
        "error"}).then(function(){
        location.reload();
      }
    );
